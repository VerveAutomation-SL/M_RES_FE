      ...
      try {
        ...
        if (result?.success && result.data) {
          ...
          if (Array.isArray(result.data)) {
            ...
            const resortWithRestaurants = result.data.find(
              (r: Restaurant) => r.id === resort.id
            );
            restaurantsData =
              resortWithRestaurants?.restaurants ||
              resortWithRestaurants?.Restaurants ||
              [];
          ...
          const resortWithRestaurants = result.find(
            (r: Restaurant) => r.id === resort.id
          );
          restaurantsData =
            resortWithRestaurants?.restaurants ||
      ...
      <div>
        ...
        <div className="space-y-2">
          {outletLoading ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              {outlets.map((outlet, index) => (
                ...
                >
                  ...
                  <span
                    className="text-sm text-gray-700 text-center truncate max-w-[7rem] sm:max-w-[10rem] overflow-hidden whitespace-nowrap"
                    title={outlet.restaurantName}
                  >
            ...
            <div className="text-sm text-gray-500 p-2">No outlets found</div>
          )}
        </div>
      </div>
    </div>
